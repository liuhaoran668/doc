...
class LeapHandRot(VecTaskRot):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # ###-------
        # asset_dof_names = self.gym.get_asset_dof_names(self.hand_asset)
        # print(f"Asset DOF Names (from asset definition): {asset_dof_names}")
        # print(f"Number of DOFs in asset: {len(asset_dof_names)}")
        # ###-----
        ...
        for i in range(num_envs):
            ...
            self.gym.enable_actor_dof_force_sensors(env_ptr, hand_actor)#关节力传感器
            # fff=self.gym.get_actor_dof_forces(env_ptr, hand_actor)#这个是获取关节力传感器的代码
            # print(fff)
            ...
            object_handle = self.gym.create_actor(env_ptr, object_asset, obj_pose, 'object', collision_group, 2, 0)
            #--------
            rsp1 = self.gym.get_actor_rigid_shape_properties(env_ptr, hand_actor)

            for ii in range(len(rsp1)):
                rsp1[ii].filter = 1  # 让手属于 filter = 1
                rsp1[ii].filter = rsp1[ii].filter & ~1  # 让手的刚体部分不能与其他手指碰撞（避免手指自碰撞）

            self.gym.set_actor_rigid_shape_properties(env_ptr, hand_actor, rsp1)
            ...
            if self.randomize_scale:
                ...
                #没有这个多级域随机化，，randomize_scale_factor
            ...
            self.gym.set_actor_scale(env_ptr, object_handle, obj_scale)#这个是动态缩放物体尺寸的代码，他不会真正的去修改urdf文件
    ...
    def reset_idx(self, env_ids):
        ...
        for n_s in range(num_scales):
            ...
            self.root_state_tensor[self.object_indices[s_ids], :7] = sampled_pose[:, 20:]
            ...
            self.object_init_pose_buf[s_ids, :] = sampled_pose[:, 20:].clone() 
    ...
    def compute_observations(self):
        ...
        if self.cfg["env"]["include_history"]:
            ...
            self.obs_buf_lag_history[at_reset_env_ids, :, 0:20] = unscale(
            ...
            if self.cfg["env"]["include_targets"]:
                self.obs_buf_lag_history[at_reset_env_ids, :, 20:40] = self.leap_hand_dof_pos[at_reset_env_ids].unsqueeze(1)
    ...
    def construct_sim_to_real_transformation(self):
        ...
        self.real_dof_order = list(range(20))
    ...
    def _create_object_asset(self):
        ...
        hand_asset_options.vhacd_params.resolution = 30000
        ...
        #     # ##------用于构建一个刚体形状索引，用于碰撞过滤操作
        #     # num_rigid_bodies=self.gym.get_asset_rigid_body_count(self.hand_asset) 
        #     # num_rigid_shapes=self.gym.get_asset_rigid_shape_count(self.hand_asset)
        #     # shape_props=self.gym.get_asset_rigid_shape_properties(self.hand_asset)
        #     # self.body_shape_indices=[]
        #     # start_index=0
        #     # for i in range(num_rigid_bodies):
        #     #     shape_count=sum(1 for shape in shape_props if shape.body==i)
        #     # #     shape_count=self.gym.get_asset_rigid_shape_count(self.hand_asset,i)
        #     #     self.body_shape_indices.append((start_index,shape_count))
        #     #     start_index+=shape_count
                     
        #         ##上面是自己加的，是得到每一个刚体的一个刚体形状数量。
    ...
    def _init_object_pose(self):
        ...
        leap_hand_start_pose.p = gymapi.Vec3(0.2, 0.05, self.cfg["env"]["leap_hand_start_z"])

        leap_hand_start_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 1, 0), -np.pi/2) 
        ...
        object_start_pose.p.x = leap_hand_start_pose.p.x + pose_dx-0.1
        object_start_pose.p.y = leap_hand_start_pose.p.y + pose_dy-0.04 #p27080是0.045
...
def compute_hand_reward(
    ...
    rotate_reward_cond = (rotation_axis[:, -1] != 0).float()#这个是判断旋转轴的最后一个张量是否为0.。。。。
    vec_dot = (object_angvel * rotation_axis).sum(-1)#   将角速度和旋转轴进行点乘，并且按照最后一个维度进行相加
    rotate_reward = torch.clip(vec_dot, max=angvel_clip_max, min=angvel_clip_min)#将旋转奖励限制在min和max之间
    rotate_reward = rotate_reward_scale * rotate_reward * rotate_reward_cond#这个就是如果旋转轴为0，则旋转奖励就会为0.
    object_linvel_penalty = torch.norm(object_linvel, p=1, dim=-1)#线速度的惩罚。
