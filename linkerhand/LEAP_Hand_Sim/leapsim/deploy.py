...
class HardwarePlayer(object):
    def __init__(self, config):
        ...
        #self.init_pose =np.array(self.config["task"]['env']['canonical_pose'])
        self.get_dof_limits()
        # self.leap_dof_lower = torch.from_numpy(np.array([
        #     -1.5720, -0.4420, -1.2220, -1.3420,  1.0192,  0.0720,  0.2520, -1.3420,
        #     -1.5720, -0.4420, -1.2220, -1.3420, -1.5720, -0.4420, -1.2220, -1.3420
        # ])).to(self.device)
    ...
    def urdf_to_sim(self, values):
        if not hasattr(self, "urdf_to_sim_indices"):
            self.construct_sim_to_real_transformation()

        return values[:, self.urdf_to_sim_indices]
    ...
    def construct_sim_to_real_transformation(self):
        ...
        self.urdf_to_sim_indices= self.config["task"]["env"]["urdf_to_real_indices"]
    ...
    def get_dof_limits(self):
        ...
        self.leap_dof_lower = [0 for _ in range(20)]
        self.leap_dof_upper = [0 for _ in range(20)]
        ### 
        ...
        for child in root.getchildren():
            if child.tag == "joint":
                ...
                if child.attrib['type']=='fixed':continue
                joint_idx=joint_counter
                joint_counter+=1
        ...
        self.leap_dof_lower = self.urdf_to_sim(self.leap_dof_lower).squeeze()
        self.leap_dof_upper = self.urdf_to_sim(self.leap_dof_upper).squeeze()
    ...
    def fetch_grasp_state(self, s=1.0):
        ...
        return grasping_states[idx][:20] # first 20 are hand dofs, last 20 is object state
    ...
    def deploy(self):
        ...
        #rospy.wait_for_service('/leap_position')#等待服务器开启
        while leap.joint_state is None:
            rospy.loginfo("Waiting for joint state topic...")
            rospy.sleep(0.01)
        ...
        for _ in range(hz * 4):
            leap.command_joint_position(self.init_pose)#这个将归一化的关节角度publish了，，，，关节映射肯定没问题
            obses, _ = leap.poll_joint_position()#这个函数就是获取实际手上的关节角度，将其转化为sim形式达到类似观测空间的作用
            ros_rate.sleep()
        print("done")
        #time.sleep(20)
        obses, _ = leap.poll_joint_position()#这个是返回真实的手部关节角度
        ...
        for i in range(num_append_iters):   
            ...
            if "phase_period" in self.config["task"]["env"]:   ###这个是实现周期性动作的一个相位控制
        ...
        #--------------z                    这个是将仿真中的观测空间给到智能体试试
        # 给定张量（假设已存在）
        # tensor_data = torch.tensor([[
        #     -9.9775e-01, -8.2898e-01,  6.5925e-01, -8.3122e-01,  9.9990e-01,
        #     -1.0000e+00,  8.3808e-01,  6.3397e-02, -1.0000e+00,  7.7941e-01,
        #     -3.0861e-01,  9.9997e-01, -8.7413e-01,  7.0077e-01, -4.7435e-01,
        #     -1.7662e-01, -3.5325e-01, -9.5279e-01,  9.4405e-01, -4.9026e-01,
        #     -1.7980e-01,  1.3425e-01,  1.3025e+00,  1.3249e-01,  1.7999e-01,
        #     -1.8190e-05,  1.4429e+00,  8.3477e-01, -1.1428e-05,  1.3968e+00,
        #     5.4274e-01,  1.8000e-01,  9.8811e-02,  1.3351e+00,  4.1263e-01,
        #     9.8171e-02, -8.7961e-01,  2.1243e-02, -4.3918e-02, -1.1699e+00,
        #     0.0000e+00,  0.0000e+00, -9.9775e-01, -8.2898e-01,  6.5925e-01,
        #     -8.3122e-01,  9.9990e-01, -1.0000e+00,  8.3808e-01,  6.3397e-02,
        #     -1.0000e+00,  7.7941e-01, -3.0861e-01,  9.9997e-01, -8.7413e-01,
        #     7.0077e-01, -4.7435e-01, -1.7662e-01, -3.5325e-01, -9.5279e-01,
        #     9.4405e-01, -4.9026e-01, -1.7980e-01,  1.3425e-01,  1.3025e+00,
        #     1.3249e-01,  1.7999e-01, -1.8190e-05,  1.4429e+00,  8.3477e-01,
        #     -1.1428e-05,  1.3968e+00,  5.4274e-01,  1.8000e-01,  9.8811e-02,
        #     1.3351e+00,  4.1263e-01,  9.8171e-02, -8.7961e-01,  2.1243e-02,
        #     -4.3918e-02, -1.1699e+00,  0.0000e+00,  0.0000e+00, -9.9775e-01,
        #     -8.2898e-01,  6.5925e-01, -8.3122e-01,  9.9990e-01, -1.0000e+00,
        #     8.3808e-01,  6.3397e-02, -1.0000e+00,  7.7941e-01, -3.0861e-01,
        #     9.9997e-01, -8.7413e-01,  7.0077e-01, -4.7435e-01, -1.7662e-01,
        #     -3.5325e-01, -9.5279e-01,  9.4405e-01, -4.9026e-01, -1.7980e-01,
        #     1.3425e-01,  1.3025e+00,  1.3249e-01,  1.7999e-01, -1.8190e-05,
        #     1.4429e+00,  8.3477e-01, -1.1428e-05,  1.3968e+00,  5.4274e-01,
        #     1.8000e-01,  9.8811e-02,  1.3351e+00,  4.1263e-01,  9.8171e-02,
        #     -8.7961e-01,  2.1243e-02, -4.3918e-02, -1.1699e+00,  0.0000e+00,
        #     1.0000e+00
        # ]], device='cuda:0')  # 直接创建张量（实际场景中可能从其他操作生成）
        
        # # 方案2：克隆张量（避免共享内存，独立副本）
        # obs_buf = tensor_data.clone()
        #print(obs_buf)
        counter = 0 
        ...
        while True:
            ...
            if hasattr(self, "actions_list"):
                ...
                #print(obs_buf.shape)
                action = self.forward_network(obs_buf)#在这个里面就是get_action
            ...
            if "disable_actions" not in self.config["task"]["env"]:
                leap.command_joint_position(commands)#将策略输出的，，，这个就是将动作发送到硬件

            ros_rate.sleep()  # keep 20 Hz command
            
            # command_list.append(commands)
            # get o_{t+1}
            obses, _ = leap.poll_joint_position()#获取相对应的硬件位置
            obses = torch.from_numpy(obses.astype(np.float32)).cuda()
            #print(f'obes:',obses)
            # obs_buf_list.append(obses.cpu().numpy().squeeze())
            cur_obs_buf = unscale(obses, self.leap_dof_lower, self.leap_dof_upper)[None]
            self.cur_obs_joint_angles = cur_obs_buf.clone()
            #print(f'cur:',cur_obs_buf.shape)#这里是获取的手部当前位置的关节角度，观测空间是手部当前位置的位姿和target位姿再加上两个时间周期
            if self.debug_viz:
                self.plot_callback()
                #如果有debug就会进入到这个代码
            ...
            if self.config["task"]["env"]["include_history"]:
                ...
                #print(f'history:',obs_buf.shape)#有his，在这里是84个元素，两个历史关节位置和时间周期
            else:
                obs_buf = torch.zeros((1, 0), device=self.device)
                #print(f'wuhistory:',obs_buf.shape)
            obs_buf = torch.cat([obs_buf, cur_obs_buf.clone()], dim=-1)
            #print(f'1:',obs_buf.shape)
            if self.config["task"]["env"]["include_targets"]:#在这里加上了目标位置，
            ...
            #print(f'2:',obs_buf.shape)
            if "obs_mask" in self.config["task"]["env"]:
                obs_buf = obs_buf * torch.tensor(self.config["task"]["env"]["obs_mask"]).cuda()[None, :]

            obs_buf = obs_buf.float()
            #print(f'3:',obs_buf.shape)
