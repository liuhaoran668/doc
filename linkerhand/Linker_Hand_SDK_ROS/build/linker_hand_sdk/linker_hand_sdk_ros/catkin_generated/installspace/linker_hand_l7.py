...
from utils.linker_hand_l7_can import LinkerHandL7Can
...
class LinkerHandL7:
    ...
    def check_left_hand(self):
        if self.left_hand_exists == True:
            if self.left_hand_joint == "L7":
                ...
                self.left_hand_can.set_torque(torque=torque)
                time.sleep(0.01)
                self.left_hand_can_send(last_pose=[255, 200, 255, 255, 255, 255, 180])
                self.left_hand_can.set_joint_speed([120,250,250,250,250,250,250])
                ColorMsg(msg="左手L7配置文件中已开启", color="green")
                ColorMsg(msg=f"当前左手L7速度为：{self.left_hand_can.get_speed()} 扭矩为:{torque}", color="green")
    ...
    def check_right_hand(self):
        if self.right_hand_exists == True:
            if self.right_hand_joint == "L7":
                ...
                self.right_hand_can.set_torque(torque=torque)
                time.sleep(0.01)
                self.right_hand_can_send(last_pose=[255, 128, 255, 255, 255, 255, 128])
                self.right_hand_can.set_joint_speed([180,250,250,250,250,250,250])
                ColorMsg(msg="右手L7配置文件中已开启", color="green")
                ColorMsg(msg=f"当前右手L7速度为：{self.right_hand_can.get_speed()} 扭矩为:{torque}", color="green")
    ...
    def position_send(self):
        while True:
            ...
            if self.left_hand_exists == True:
                # 发送左手数据
                if self.left_hand_joint == "L7":
                    self.left_hand_can_send(last_pose=self.left_hand_position)
                # 发送左手状态
            ...
            # 发送右手数据
                if self.right_hand_joint == "L7":
                    self.right_hand_can_send(last_pose=self.right_hand_position)
                #发送右手状态
    ...
    # L7发送can数据
    def left_hand_can_send(self,last_pose):
        self.left_hand_can.set_joint_positions(last_pose)
    # L7发送can数据
    def right_hand_can_send(self,last_pose):
            ...
            if self.left_hand_joint == "L7":
                ...
                data = {
                    "left_hand":{
                        ...
                        "torque": self.left_hand_can.get_torque()
                    }
                }
            ...
            if self.right_hand_joint == "L7":
                ...
                data = {
                    "right_hand":{
                        ...
                        "torque": self.right_hand_can.get_torque()
                    }
                }
    ...
    def hand_setting_cb(self, msg):
        ...
        if data["setting_cmd"] == "set_max_torque_limits": # 设置最大扭矩
            ...
            if data["params"]["hand_type"] == "left":
                if self.left_hand_joint == "L7":# 判断是否为L7
                    torque = int(data["params"]["torque"])
                    ColorMsg(msg=f"设置L7左手最大扭矩为{torque}", color="yellow")
                    #hand.set_max_torque_limits(pressures=[torque] * 7, type="set")
                    hand.set_torque(torque=torque)
            if data["params"]["hand_type"] == "right":
                if self.right_hand_joint == "L7":# 判断是否为L7
                    torque = int(data["params"]["torque"])
                    ColorMsg(msg=f"设置L7左手最大扭矩为{torque}", color="yellow")
                    #hand.set_max_torque_limits(pressures=[torque] * 7, type="set")
                    hand.set_torque(torque=torque)
        ...
        if data["setting_cmd"] == "set_speed": # 设置速度
            if isinstance(data["params"]["speed"], list) == True:
                ...
                speed = [int(data["params"]["speed"])] * 7
            if hand_left == True and self.left_hand_joint == "L7":
                hand.set_joint_speed(speed=speed)
                speed = hand.get_speed()
                ColorMsg(msg=f"设置L7左手速度为{speed}", color="yellow")
            elif hand_right == True and self.right_hand_joint == "L7":
                hand.set_joint_speed(speed=speed)
                speed = hand.get_speed()
                ColorMsg(msg=f"设置L7右手速度为{speed}", color="yellow")

        
        if data["setting_cmd"] == "clear_faults": # 清除错误
            if hand_left == True and self.left_hand_joint == "L7" :
                ColorMsg(msg=f"L7左手不能清除错误")
            elif hand_right == True and self.right_hand_joint == "L7" :
                ColorMsg(msg=f"L7右手不能清除错误")
            else:
                hand.clear_faults()
        if data["setting_cmd"] == "get_faults": # 获取错误
            if hand_left == True and self.left_hand_joint == "L7" :
                l7_fault = hand.get_fault()
                ColorMsg(msg=f"L7左手当前错误码: {l7_fault}", color="yellow")
            elif hand_right == True and self.right_hand_joint == "L7" :
                l7_fault = hand.get_fault()
                ColorMsg(msg=f"L7右手当前错误码: {l7_fault}", color="yellow")
            else:
                hand.get_faults()
        if data["setting_cmd"] == "electric_current": # 获取电流
            if hand_left == True and self.left_hand_joint == "L7" :
                ColorMsg(msg=f"L7左手不能获取电压")
            elif hand_right == True and self.right_hand_joint == "L7" :
                ColorMsg(msg=f"L7右手不能获取电压")
            else:
