...
class LinkerHandL25:
    ...
    def init_right_hand(self):
        if self.right_hand_exists == True and self.right_hand_joint == "L25":
            ...
            if self.motor_mode == "disability":
                ...
                # 设置手指速度0~255
                self.right_hand.set_speed(speed=[255, 255, 255, 255, 255, 255])
                # 设置手掌张开
                ...
                self.right_hand_status_pub = rospy.Publisher("/cb_right_hand_state",JointState,queue_size=1)#发布消息类型。
    ...
    def hand_status(self):
        ...
        if self.right_hand != None:
            ...
            #print(len(right_hand_state))usb和命令窗口必须都拔掉关闭，usb通信会有一个管道，Failed to send message: Failed to transmit: Network is down [Error Code 100]
            #这个的意思就是缺失管道，重新拔一下usb就可以
